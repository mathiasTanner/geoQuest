...
import {
  ...
  evaluateWordsearchSelectionForOwnedQuest,
...
export async function POST(request: NextRequest) {
  ...
  const rateLimit = await takeRateLimitHit(
    `wordsearch-evaluate:${rateLimitKey}`,
    180,
    5 * 60 * 1000
  );
  ...
  try {
    const evaluation = await evaluateWordsearchSelectionForOwnedQuest(
      session.session,
      body.questAccessId,
      body.stepDocumentId,
      body.foundWordIds,
      {
        row: Number(body.startRow),
        col: Number(body.startCol),
      },
      {
        row: Number(body.endRow),
        col: Number(body.endCol),
      }
    );
